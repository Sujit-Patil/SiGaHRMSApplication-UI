...
export class AdminChartComponent {
  ...
  getProject() {
    ...
    this.apiService.getByDate(Api.LeaveRequest, new RequestDto(null, transformedDate, null)).subscribe((data: LeaveRequest[]) => {
      this.pendingleaveRequest = data.filter((leave) => leave.LeaveRequestStatus == LeaveRequestStatus.Open);
      this.onLeaveRequest=data.filter((leave) => leave.LeaveRequestStatus == LeaveRequestStatus.Approved);
    });
  }
  ...
  getTimeSheetBilling(data: TimeSheetDetail[], month = false) {
    ...
    data.forEach((item) => {
      const projectTitle = item.Task.Project.Title;
      if (!arrays.has(projectTitle)) {
